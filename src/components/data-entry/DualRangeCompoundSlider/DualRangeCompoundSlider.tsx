...
import './DualRangeCompoundSlider.css';
...
export const DualRangeCompoundSlider: React.FC<Props> = ({
  domain = [0, 100],
  step = 1,
  values = domain.slice(),
  onChange = undefined
  ...
  return (
    <div className="slider" style={{ height: 50, width: '100%' }}>
      <Slider
        mode={3}
        step={step}
        domain={domain}
        ...
        onUpdate={(value: ReadonlyArray<number>) =>
          setUpdate(
            values.map((val) => {
              return parseFloat(val.toFixed(decimals));
            })
